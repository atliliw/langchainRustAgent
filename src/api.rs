...
use crate::qdrant_store::{QdrantStore, StoreError};
use crate::bm25_store::{BM25Store, BM25Error};
use crate::hybrid_store::{HybridStore};
use crate::document_processor::{DocumentProcessor, ProcessError};
use crate::conversation_store::{ConversationStore, ConversationError, ChatRequest, ChatResponse, SessionInfo, SourceInfo, ConversationMessage};
...
pub enum ApiError {
    ...
    #[error("混合检索失败: {0}")]
    HybridError(#[from] crate::hybrid_store::HybridError),
...
impl ApiService {
    ...
    pub async fn chat(&self, request: ChatRequest) -> Result<ChatResponse, ApiError> {
        use crate::conversation_store::SearchMode;
...
pub async fn get_conversation_history(&self, session_id: &str) -> Result<Vec<ConversationMessage>, ApiError> {
    ...
    pub async fn list_documents(&self) -> Result<Vec<crate::bm25_store::DocumentInfo>, ApiError> {
    ...
    pub fn get_langgraph_info() -> serde_json::Value {
        crate::langgraph_demo::LangGraphDemoService::new().get_graph_info()
    }
    
    pub async fn run_langgraph_parallel(input: String) -> Result<crate::langgraph_demo::ParallelDemoResult, ApiError> {
        let service = crate::langgraph_demo::LangGraphDemoService::new();
        service.run_parallel_demo(input).await.map_err(|e| ApiError::SearchError(e.to_string()))
    }
    
    pub async fn run_langgraph_conditional(input: String) -> Result<crate::langgraph_demo::ConditionalDemoResult, ApiError> {
        let service = crate::langgraph_demo::LangGraphDemoService::new();
        service.run_conditional_demo(input).await.map_err(|e| ApiError::SearchError(e.to_string()))
    }
    
    pub async fn run_langgraph_stream(input: String) -> Result<Vec<crate::langgraph_demo::StreamDemoEvent>, ApiError> {
        let service = crate::langgraph_demo::LangGraphDemoService::new();
